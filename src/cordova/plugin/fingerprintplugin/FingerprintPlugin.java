...
public class FingerprintPlugin extends CordovaPlugin {
	...
	public void initialize(CordovaInterface cordova, CordovaWebView webView) {
		...
		try {
			...
			_cipher = Cipher.getInstance(KeyProperties.KEY_ALGORITHM_AES + "/" + KeyProperties.BLOCK_MODE_CBC + "/"
		...
		if (action.equals(IS_AVAILABLE)) {
			if (isHardwareDetected()) {
				if (hasEnrolledFingerprints()) {
					...
					String errorMessage = createErrorMessage(NO_FINGERPRINT_ENROLLED_CODE,
							NO_FINGERPRINT_ENROLLED_MESSAGE);
			...
			SharedPreferences sharedPref = _cordova.getActivity().getApplicationContext()
	 ...
	 * the user has authenticated with fingerprint.
	 */
	public static boolean createKey() {
		String errorMessage = "";
		String createKeyExceptionErrorPrefix = "Failed to create key: ";
		boolean isKeyCreated = false;
		// The enrolling flow for fingerprint. This is where you ask the user to set up
		// fingerprint
		...
		try {
			...
			_keyGenerator.init(new KeyGenParameterSpec.Builder(CLIENT_ID,KeyProperties.PURPOSE_ENCRYPT | KeyProperties.PURPOSE_DECRYPT)
	 ...
	 * {@link #createKey(boolean setUserAuthenticationRequired)} method.
	 *
	 * @return {@code true} if initialization is successful, {@code false} if the
	 *         lock screen has been disabled or reset after the key was generated,
	...
	private boolean initCipher(int mode) {
		...
		try {
			...
			if (mode == Cipher.ENCRYPT_MODE) {
				...
				SharedPreferences sharedPref = _cordova.getActivity().getApplicationContext().getSharedPreferences(SHARED_PREFS_NAME, Context.MODE_PRIVATE);
				byte[] ivBytes = Base64.decode(sharedPref.getString("fing_iv" + _keyID, ""), Base64.DEFAULT);
	...
	public void onAuthenticated() {
		...
		try {
			...
			if (_currentMode == Cipher.DECRYPT_MODE) {
				...
				editor.putString("fing_iv" + _keyID, Base64.encodeToString(_cipher.getIV(), Base64.DEFAULT));
	...
	public void startListening(FingerprintManager.CryptoObject cryptoObject, CallbackContext callbackContext) {
		...
		_fingerPrintManager.authenticate(cryptoObject, _cancellationSignal, 0, new FingerprintManager.AuthenticationCallback() {
			@Override
			public void onAuthenticationError(int errMsgId, CharSequence errString) {
				stopListening();
				callbackContext.error("onAuthenticationError " + errString.toString());
			}
			...
			public void onAuthenticationHelp(int helpMsgId, CharSequence helpString) {
				stopListening();
				callbackContext.error("onAuthenticationHelp " + helpString.toString());
			}
